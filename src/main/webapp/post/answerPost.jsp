...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
	<div class="row">
		<div class="col-sm-8 blog-main"> <br>
			<h2 class="sub-header">답글등록</h2>
			
			<div class="table-responsive">
				<form action="${cp }/createAnswer?board_no=${boardVo.board_no}" method="post" enctype="multipart/form-data">
					<select disabled>
						<c:forEach var="board" items="${boardList }">
							<c:if test="${board.board_no == boardVo.board_no }">
								<option data-board_no="${board.board_no }" selected="selected" value="${board.board_no }">${board.board_title }</option>
							</c:if>
						</c:forEach>
					</select>
					<br><br>
